...
class FeatureComputer(BaseEstimator, TransformerMixin):
    ...
    def transform(self, df, drop_first_n: Optional[int] = 50):
        ...
        return feat[FEATURES + ["activity_id", WEIGHT_COL]]
...
class PacePredictorPipeline:
    ...
    def init_pipeline(self, label_id):
        ...
        return self.next_step_pred_pipeline
...
def weight_data(df):
    df[WEIGHT_COL] = 1
    from data.local_db.manual.labelled_runs import races

    df.loc[df["activity_id"].isin(races["activity_id"]), WEIGHT_COL] = 20
    return df
